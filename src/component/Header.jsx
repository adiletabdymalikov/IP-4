...
const Header = () => {
  ...
  const searchF = async () => {
    ...
    let seacrData = await axios({
      url: `https://api.themoviedb.org/3/search/multi?api_key=${apikey}&language=${apiLang}&query=${inp}&page=1&include_adult=false`
    })
      ...
      {searchstate != null ? <>
        <div className="col-lg-12 seacrh">
          {personSearch != null ? personSearch.map(item => <>
            <h6 className='my-3'> <i className="fa-solid fa-user mx-3"></i> <a href={"/persondetail/" + item.id}> {item.name} </a></h6>
          </>) : <></>}
          {moviSearch != null ?
            moviSearch.map(i =>
              <>
                <h6 className='my-3'><i className="mx-3 fa-solid fa-video"></i> <a href={"/detail/" + i.id}>{i.title}</a>  </h6>
              </>) : <></>}
        </div>
      </> : <></>}
    </div>
